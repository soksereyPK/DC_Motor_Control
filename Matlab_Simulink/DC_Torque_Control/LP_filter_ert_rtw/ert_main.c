...
void rt_OneStep(void)
{
   ...
   */
  LP_filter_SetEventsForThisBaseStep(eventFlags);
  ...
  currentTime = (extmodeSimulationTime_T) ((LP_filter_M->Timing.clockTick0 * 1)
    + 0)
    ;
  LP_filter_step0();
  ...
  if (need2runFlags[2]) {
    ...
    {
     case 2 :
      currentTime = (extmodeSimulationTime_T) ((LP_filter_M->Timing.clockTick2 *
        10) + 0)
        ;
      LP_filter_step2();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(LP_filter_M, 0);
  ...
  LP_filter_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(LP_filter_M->extModeInfo, &rtmGetTFinal(LP_filter_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(LP_filter_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(LP_filter_M);
