...
void Current_test_step0(void)          /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (Current_test_DW.obj_f.SampleTime != Current_test_P.Encoder1_SampleTime) {
    Current_test_DW.obj_f.SampleTime = Current_test_P.Encoder1_SampleTime;
  }

  if (Current_test_DW.obj_f.TunablePropsChanged) {
    Current_test_DW.obj_f.TunablePropsChanged = false;
  }

  MW_EncoderRead(Current_test_DW.obj_f.Index, &rtb_Encoder1_0);
   ...
   *  MATLABSystem: '<S3>/Encoder1'
   */
  Current_test_B.Gain = Current_test_P.Gain2_Gain * (real_T)rtb_Encoder1_0 *
    Current_test_P.Gain_Gain;

  /* MATLAB Function: '<S3>/MATLAB Function4' incorporates:
   *  Sin: '<Root>/Sine Wave2'
   */
  pwm_cal = (sin(Current_test_P.SineWave2_Freq * Current_test_M->Timing.t[0] +
                 Current_test_P.SineWave2_Phase) * Current_test_P.SineWave2_Amp
             + Current_test_P.SineWave2_Bias) * 254.0 / 12.0;
  if (pwm_cal > 0.0) {
    rtb_IN1 = 1;
    rtb_IN2 = 0;
  } else if (pwm_cal < 0.0) {
  ...
  /* MATLABSystem: '<S3>/PWM' */
  obj = &Current_test_DW.obj_kg;
  obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_GetHandle(4UL);
  if (!(pwm_cal <= 255.0)) {
    pwm_cal = 255.0;
  }

  MW_PWM_SetDutyCycle(Current_test_DW.obj_kg.PWMDriverObj.MW_PWM_HANDLE, pwm_cal);
...
void Current_test_step2(void)          /* Sample time: [0.1s, 0.0s] */
{
   ...
   *  MATLABSystem: '<Root>/Current Reg read'
   */
  Current_test_B.Gain1 = (real_T)((int32_T)Current_test_P.Gain_Gain_j * b_output)
...
void Current_test_initialize(void)
{
  ...
  /* External mode info */
  Current_test_M->Sizes.checksums[0] = (3723016616U);
  Current_test_M->Sizes.checksums[1] = (543604689U);
  Current_test_M->Sizes.checksums[2] = (1954372507U);
  Current_test_M->Sizes.checksums[3] = (2451984152U);
...

  {
    ...
    Current_test_DW.obj_f.Index = 0U;
    ...
    MW_EncoderSetup(2UL, 3UL, &Current_test_DW.obj_f.Index);
    Current_test_DW.obj_f.isSetupComplete = true;
    Current_test_DW.obj_f.TunablePropsChanged = false;

    /* InitializeConditions for MATLABSystem: '<S3>/Encoder1' */
    MW_EncoderReset(Current_test_DW.obj_f.Index);
    ...
    Current_test_DW.obj_mm.matlabCodegenIsDeleted = false;
    Current_test_DW.obj_mm.isInitialized = 1L;
    digitalIOSetup(7, 1);
    Current_test_DW.obj_mm.isSetupComplete = true;

    /* Start for MATLABSystem: '<S3>/PWM' */
    Current_test_DW.obj_kg.matlabCodegenIsDeleted = true;
    Current_test_DW.obj_kg.isInitialized = 0L;
    Current_test_DW.obj_kg.matlabCodegenIsDeleted = false;
    obj = &Current_test_DW.obj_kg;
    Current_test_DW.obj_kg.isSetupComplete = false;
    Current_test_DW.obj_kg.isInitialized = 1L;
    obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_Open(4UL, 0.0, 0.0);
    Current_test_DW.obj_kg.isSetupComplete = true;
...
void Current_test_terminate(void)
{
  ...
  if (!Current_test_DW.obj_mm.matlabCodegenIsDeleted) {
    Current_test_DW.obj_mm.matlabCodegenIsDeleted = true;
  }
  ...
  obj = &Current_test_DW.obj_kg;
  if (!Current_test_DW.obj_kg.matlabCodegenIsDeleted) {
    Current_test_DW.obj_kg.matlabCodegenIsDeleted = true;
    if ((Current_test_DW.obj_kg.isInitialized == 1L) &&
        Current_test_DW.obj_kg.isSetupComplete) {
      obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_GetHandle(4UL);
      MW_PWM_SetDutyCycle(Current_test_DW.obj_kg.PWMDriverObj.MW_PWM_HANDLE, 0.0);
      obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_GetHandle(4UL);
      MW_PWM_Close(Current_test_DW.obj_kg.PWMDriverObj.MW_PWM_HANDLE);
    }
  }
