...
int main(void)
{
  ...
  hw_ekf_est_all_para_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(hw_ekf_est_all_para_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(hw_ekf_est_all_para_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(hw_ekf_est_all_para_M);
