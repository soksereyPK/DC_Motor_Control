...
{
  {
    ...
    MW_EncoderRead(Torque_control_DW.obj.Index, &rtb_IN1);

    /* Gain: '<S1>/Gain2' incorporates:
     *  DataTypeConversion: '<S1>/Data Type Conversion1'
     *  MATLABSystem: '<S1>/Encoder1'
     */
    Torque_control_B.Gain2 = Torque_control_P.Gain2_Gain * (real_T)rtb_IN1;

    /* Sum: '<Root>/Sum3' incorporates:
     *  Constant: '<Root>/Constant5'
     */
    Torque_control_B.Sum3 = Torque_control_P.Constant5_Value -
      Torque_control_B.Gain2;
    ...
    writeDigitalPin(15, (uint8_T)rtb_IN1);

    /* MATLABSystem: '<S1>/Digital Output1' */
    writeDigitalPin(5, (uint8_T)rtb_IN2);

    /* MATLABSystem: '<S1>/PWM' */
    obj = &Torque_control_DW.obj_ir;
    obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_GetHandle(18U);
    if (!(pwm_cal <= 255.0)) {
      pwm_cal = 255.0;
    }
...
void Torque_control_initialize(void)
{
  ...
  /* External mode info */
  Torque_control_M->Sizes.checksums[0] = (4097939379U);
  Torque_control_M->Sizes.checksums[1] = (986202651U);
  Torque_control_M->Sizes.checksums[2] = (2063811960U);
  Torque_control_M->Sizes.checksums[3] = (439775687U);
...

  {
    ...
    MW_EncoderSetup(13U, 14U, &Torque_control_DW.obj.Index);
    ...
    Torque_control_DW.obj_i.isInitialized = 1;
    digitalIOSetup(15, 1);
    ...
    Torque_control_DW.obj_e.isInitialized = 1;
    digitalIOSetup(5, 1);
    ...
    obj = &Torque_control_DW.obj_ir;
    Torque_control_DW.obj_ir.isInitialized = 1;
    obj->PWMDriverObj.MW_PWM_HANDLE = MW_PWM_Open(18U, 0.0, 0.0);
...
void Torque_control_terminate(void)
{
  ...
  if (!Torque_control_DW.obj_ir.matlabCodegenIsDeleted) {
    ...
    if ((Torque_control_DW.obj_ir.isInitialized == 1) &&
