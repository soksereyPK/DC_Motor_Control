###########################################################################
## Makefile generated for component 'posi'. 
## 
## Makefile     : posi.mk
## Generated on : Mon Jun 05 16:53:09 2023
## Final product: $(RELATIVE_PATH_TO_ANCHOR)/posi.elf
...
PRODUCT = $(RELATIVE_PATH_TO_ANCHOR)/posi.elf
...
DEFINES_CUSTOM = 
DEFINES_OPTS = -DXCP_DAQ_SUPPORT -DXCP_CALIBRATION_SUPPORT -DXCP_TIMESTAMP_SUPPORT -DXCP_TIMESTAMP_BASED_ON_SIMULATION_TIME -DXCP_SET_MTA_SUPPORT -DXCP_MEM_DAQ_RESERVED_POOLS_NUMBER=2 -DEXTMODE_XCP_TRIGGER_SUPPORT -DEXTMODE_STATIC -DEXTMODE_STATIC_SIZE=24576 -DON_TARGET_WAIT_FOR_START=1 -DTID01EQ=1 -DXCP_MEM_BLOCK_1_SIZE=16 -DXCP_MEM_BLOCK_1_NUMBER=1 -DXCP_MEM_BLOCK_2_SIZE=32 -DXCP_MEM_BLOCK_2_NUMBER=1 -DXCP_MEM_BLOCK_3_SIZE=48 -DXCP_MEM_BLOCK_3_NUMBER=1
...
DEFINES_STANDARD = -DMODEL=posi -DNUMST=2 -DNCSTATES=0 -DHAVESTDIO -DMODEL_HAS_DYNAMICALLY_LOADED_SFCNS=0
...
OBJS = MW_ArduinoEncoder.o MW_PWM.o MW_PWMDriver.o ArduinoPinHandleMap.o xcp_ext_mode.o posi.o posi_data.o rtGetInf.o rtGetNaN.o rt_nonfinite.o xcp_ext_common.o xcp_ext_classic_trigger.o xcp.o xcp_standard.o xcp_daq.o xcp_calibration.o xcp_fifo.o xcp_transport.o xcp_mem_default.o xcp_drv_rtiostream.o xcp_frame_serial.o xcp_ext_param_default_serial.o arm_m3_cortex_handler.o MW_ArduinoHWInit.o io_wrappers.o arduinoARMScheduler.o m3m4m4f_multitasking.o rtiostream_serial_daemon.o sys_arch.o
...
%.o : $(START_DIR)/posi_ert_rtw/%.c
	$(CC) $(CFLAGS) -o "$@" "$<"


%.S.o : $(START_DIR)/posi_ert_rtw/%.s
	$(AS) $(ASFLAGS) -o "$@" "$<"
...
%.o : $(START_DIR)/posi_ert_rtw/%.cpp
	$(CPP) $(CPPFLAGS) -o "$@" "$<"


%.o : $(START_DIR)/posi_ert_rtw/%.cc
	$(CPP) $(CPPFLAGS) -o "$@" "$<"
...
ert_main.o : $(START_DIR)/posi_ert_rtw/ert_main.c
	$(CC) $(CFLAGS) -o "$@" "$<"


posi.o : $(START_DIR)/posi_ert_rtw/posi.c
	$(CC) $(CFLAGS) -o "$@" "$<"


posi_data.o : $(START_DIR)/posi_ert_rtw/posi_data.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rtGetInf.o : $(START_DIR)/posi_ert_rtw/rtGetInf.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rtGetNaN.o : $(START_DIR)/posi_ert_rtw/rtGetNaN.c
	$(CC) $(CFLAGS) -o "$@" "$<"


rt_nonfinite.o : $(START_DIR)/posi_ert_rtw/rt_nonfinite.c
	$(CC) $(CFLAGS) -o "$@" "$<"
