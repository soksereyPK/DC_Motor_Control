...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) ReadEnc_SPI_M->Timing.clockTick0;
  ReadEnc_SPI_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ReadEnc_SPI_M, 0);
  ...
  ReadEnc_SPI_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(ReadEnc_SPI_M->extModeInfo, &rtmGetTFinal
    (ReadEnc_SPI_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ReadEnc_SPI_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ReadEnc_SPI_M);
